...
def saveToCSV(directory, current_file, results, numCells):
    ...
    filepath = os.path.join(str(directory), "results-" + str(filei) + ".csv")
...
if folder is None:
    ...
    for (root, dirs, files) in os.walk(folder):
        for file in files:
            if file.endswith(".nd") or file.endswith(".nd2") or file.endswith(".czi"):
    ...
    while True:
        filei += 1 
        filepath = os.path.join(str(folder), "results-" + str(filei)+ ".csv")
        print(filepath)
        #filepath = bytes(filepath,"utf-8").decode("unicode_escape"
        #print("CHECKING FILE ... ", filepath)
       
        print("FOLDER RAW:", repr(folder))
        print("FILEPATH RAW:", repr(filepath))
        print("EXISTS FOLDER:", os.path.exists(folder))
        print("EXISTS FILE:", os.path.exists(filepath))
       
        with open(str(filepath), 'a+b') as file:
...
From: Asaf Ben LEBOVIC <asaf-ben.lebovic@pasteur.fr>
Sent: Thursday, February 19, 2026 4:29 PM
To: Eva LEGRAND <eva.legrand@pasteur.fr>
Subject: Re:
...
From: Eva LEGRAND <eva.legrand@pasteur.fr>
Date: Thursday, February 19, 2026 at 4:24 PM
To: Asaf Ben LEBOVIC <asaf-ben.lebovic@pasteur.fr>
Subject: <no subject>
...
if folder is None:
    ...
    while True:
        filei += 1 
        filepath =  "results-"+str(filei)+".csv"
        filepath = os.path.join(str(folder),filepath)
        print(filepath)
        #filepath = bytes(filepath,"utf-8").decode("unicode_escape"
        #print("CHECKING FILE ... ", filepath)
